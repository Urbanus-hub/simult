...
import { Send, Image as ImageIcon, Paperclip, MoreVertical, Search, Phone, Video } from "lucide-react"
...
import { getRoomMessages, getDirectMessages, sendMessage } from "@/services/messageServices"
        ...
        if (type === 'room' && (message as any).room === id) {
             setMessages(prev => [...prev, message])
                  ...
                  ((message.sender._id === id) || // Message from them to me
                   (message.sender._id === user?.id && (message as any).recipient === id))) { // Message from me to them (should be handled by optimistic UI, but good for sync)
    ...
    <div className="flex flex-col h-full bg-background">
        ...
        <div className="flex flex-col gap-4">
            {loading && (
                <div className="flex justify-center p-4">
                    <span className="text-xs text-muted-foreground animate-pulse">Loading messages...</span>
                </div>
            )}
            ...
            {messages.map((msg, index) => {
                ...
                return (
                    <div key={msg._id} className={cn("flex gap-2 max-w-[80%]", isMe ? "ml-auto" : "")}>
                        {!isMe && (
                             <div className="w-8 shrink-0">
                                {showAvatar && (
                                    <Avatar className="h-8 w-8">
                                        <AvatarImage src={msg.sender.avatar} />
                                        <AvatarFallback>{msg.sender.username.substring(0, 1).toUpperCase()}</AvatarFallback>
                                    </Avatar>
                                )}
                             </div>
                        )}
                        <div className={cn(
                            ...
                            {!isMe && showAvatar && (
                                <p className="text-[10px] opacity-70 mb-1 font-semibold">{msg.sender.displayName || msg.sender.username}</p>
                            )}
      ...
      <div className="p-4 border-t">
        <div className="flex items-end gap-2">
            ...
            />
            <Button onClick={handleSend} disabled={!inputText.trim()} className="shrink-0">
